...
function frontera(from) {
  ...
  return `<section class="section" id="frontera"><p class="eyebrow">Qué se comparte y qué no</p><h2>Frontera con ${other}</h2><p class="section-intro">Separar los enfoques no significa duplicar el producto. Lo que se comparte es el núcleo técnico; lo que diverge es casi todo lo comercial.</p><div class="grid grid--2"><div class="card card--strong"><h3>Compartido</h3>${bullets([
    ...
    "Plataforma, persistencia, revisión de excepciones y exportación; sincronización offline solo donde el despliegue la requiera.",
...
function fondosPage() {
  ...
  <main>
    <div class="callout"><strong>POC Mini 4K · 7 sep 2026:</strong> electricidad permanente e internet por Starlink garantizados. Se recomienda procesamiento en nube; las alternativas offline de este dossier corresponden a otros despliegues. <a href="supuestos.html#s06">Ver supuesto actualizado</a>.</div>
