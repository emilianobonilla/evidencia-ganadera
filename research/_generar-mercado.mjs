...
const items = [
  {
    ...
    questions: ["¿Entrega RFID, frames originales, video continuo, GPS y sello de dispositivo por evento?", "¿Cómo evita doble lectura, lectura vecina o reutilización de un video?", "¿Acepta una comparación ciega con HDX SNIG y balanza calibrada en Uruguay?"],
    refs: ["olho","olhoFdc","snig"],
  },
  {
    n: "02", slug: "ganaderia", name: "GanaderIA", group: "direct", groupLabel: "Competidor directo por lote", origin: "Argentina", maturity: "Comercial con validación de campo", action: "Vigilar y explorar complementariedad",
    thesis: "Reduce mucho el costo de inventario y estimación de peso por lote desde dron, pero no demuestra identidad individual SNIG ni cadena de custodia.",
    evidence: [
    ...
    caps: {count:["yes","Fuerte"], identity:["unknown","RFID no publicado"], weight:["yes","Estimado por dron"], location:["partial","Operación geográfica, no prueba"], visual:["yes","Video aéreo"], provenance:["unknown","No publicada"], offline:["unknown","Procesamiento/plataforma"], uruguay:["partial","Regional; despliegue UY no confirmado"]},
    overlap: "Compite en conteo, inventario periódico y tendencia de peso por lote, especialmente fuera del feedlot. Puede satisfacer buena parte de una auditoría de existencia a menor manejo.",
    ...
    strategy: "Tratarlo como módulo de cobertura de lote. Comparar costo/tiempo y explorar si sus videos y resultados pueden incorporarse a un paquete de evidencia con RFID terrestre.",
    risks: ["El estudio es una prueba de concepto descriptiva, no validación estadística concluyente.", "La calidad cae con aglomeración, viento y protocolo subóptimo.", "La app comercial autónoma no fue la que produjo los resultados publicados."],
    questions: ["¿Puede conservar video original, telemetría del dron y hash de carga?", "¿Existe API para resultados por animal y máscaras/frames?", "¿Cómo se enlaza un resultado aéreo con un RFID leído en tierra?"],
    refs: ["ganader","validagro"],
  },
  {
  ...
  },
  {
    ...
    caps: {count:["partial","Muestra de visitas"], identity:["yes","EID"], weight:["yes","Parcial → total"], location:["partial","Unidad en potrero; GPS no publicado"], visual:["no","No es parte del flujo"], provenance:["unknown","Registro cloud ordinario"], offline:["partial","Unidad autónoma; resultado remoto"], uruguay:["yes","Presencia declarada"]},
    ...
    strategy: "Probarlo como línea base o incluso como sensor de peso dentro del sistema de evidencia. Validar HDX SNIG, soporte local, tasa de cobertura y acceso a datos crudos/API.",
    risks: ["No pesa a todo el lote en cada ventana.", "Datos y cálculo dependen de nube/conectividad declarada.", "Compatibilidad concreta con caravanas SNIG y soporte uruguayo deben confirmarse."],
    questions: ["¿Qué unidades y clientes existen en Uruguay?", "¿Lee las caravanas HDX actualmente entregadas por SNIG?", "¿Exporta lecturas crudas, rechazos, ID de equipo y timestamps firmados?"],
    refs: ["opti","optiFaq","optiStory","snig"],
  },
  {
    ...
    risks: ["Disponibilidad, costo y soporte regional no publicados.", "La validación es sólida pero contextual; requiere prueba con razas y manejo local.", "HDX declarado no garantiza compatibilidad plug-and-play con SNIG."],
    ...
    refs: ["vytelle","vytellePaper","snig"],
  },
  {
  ...
  },
  {
    ...
    strategy: "Tomarlo como patrón de integración y experiencia offline. Mantener Smart Weight como fuente de eventos especializados y exportables.",
  ...
  },
  {
    ...
    strategy: "No replicar GPS en la primera etapa. Probar una muestra centinela o usarlo para validar inferencias de ubicación y relación.",
  ...
  },
  {
    ...
    thesis: "No mide animales, pero muestra cómo convertir EID e historia en documentación aceptada por un tercero; es la referencia más útil para la segunda etapa financiera.",
    evidence: [
...
function footer(prefix = "") {
  return `<footer class="footer">Dossier vivo · versión 0.4 · corte 23 ago 2026 · <a href="${prefix}supuestos-mercado.html">supuestos de mercado</a> · <a href="${prefix}preguntas-mercado.html">preguntas de diligencia</a></footer>`;
}
...
function itemPage(item) {
  ...
  <main>
    <section class="hero hero--compact">
      ...
      <aside class="meta-card"><dl><dt>Origen</dt><dd>${esc(item.origin)}</dd><dt>Madurez</dt><dd>${esc(item.maturity)}</dd><dt>Acción</dt><dd>${esc(item.action)}</dd><dt>Corte</dt><dd>23 ago 2026</dd></dl></aside>
    </section>
    <div class="layout">
...
function marketPage() {
  ...
  return `${head("Mapa competitivo · Ganadería aumentada", "Competidores, sustitutos, similares y candidatos de convergencia para Smart Weight.")}${nav("mercado")}
  <main>
    <section class="hero"><div><p class="eyebrow">Documento 04 · Mercado</p><h1>La competencia viene por piezas.</h1><p class="lede">Quince fichas separan captura de peso, gestión, ubicación y confianza. El hueco no es “otra balanza”: es un evento RFID–foto–peso–ubicación cuya procedencia pueda defenderse.</p></div><aside class="meta-card"><dl><dt>Versión</dt><dd>0.4</dd><dt>Ítems</dt><dd>15</dd><dt>Geografía</dt><dd>Uruguay + comparables</dd><dt>Corte</dt><dd>23 ago 2026</dd><dt>Orientación</dt><dd>Construir / integrar / aliar</dd></dl></aside></section>
    <div class="layout"><aside class="toc"><strong>Contenido</strong><a href="#respuesta">Respuesta ejecutiva</a><a href="#matriz">Matriz</a><a href="#direct">Directos</a><a href="#substitute">Sustitutos</a><a href="#similar">Similares</a><a href="#evolution">Evolutivos</a><a href="#metodo">Método</a></aside><article class="content">
      <section class="section" id="respuesta"><p class="eyebrow">Respuesta ejecutiva</p><h2>Cinco decisiones que deja el mapa</h2><div class="grid grid--2">
        ...
        <div class="card card--warm"><span class="number">2</span><h3>El peso frecuente ya tiene mercado</h3><p>Optiweigh, Vytelle y Tru‑Test WOW resuelven RFID + tendencia con baja intervención; Optiweigh declara presencia en Uruguay.</p></div>
        <div class="card"><span class="number">3</span><h3>El dron domina cobertura, no identidad</h3><p>GanaderIA ofrece una ruta potente para conteo/lote, con límites de pesaje transparentes y sin enlace SNIG público.</p></div>
        <div class="card"><span class="number">4</span><h3>No construir otro ERP ganadero</h3><p>Baqueano, PGG, Finca y AgriWebb ya cubren registro e historia. Smart Weight debe ser una fuente de evidencia integrable.</p></div>
        <div class="card card--strong"><span class="number">5</span><h3>La ventaja puede estar en la prueba</h3><p>CattleProof muestra que captura, registro inalterable y verificación independiente son capas distintas. Esa separación es clave para la auditoría financiera.</p></div>
      </div><div class="callout"><strong>Posicionamiento sugerido:</strong> “captura verificable de existencia, identidad, tendencia de peso y ubicación” para decisiones productivas primero y evidencia financiera después.</div>
...
const marketAssumptions = [
  ...
  ["M03","Crítico","La correspondencia RFID–imagen puede superar 99,9% o abstenerse cuando hay duda.","Un error de identidad contamina historia y auditoría.","Prueba deliberada con animales juntos, doble lectura, vueltas atrás y tag ausente.","Si no puede demostrarse, separar peso visual de historia individual."],
  ["M04","Alto","La evidencia anclada al dispositivo tiene valor económico para un tercero.","La etapa financiera sólo se justifica si reduce inspección, fraude o costo de capital.","Entrevistas con bancos, fondos, fiduciarios y auditores usando un paquete de muestra.","Si no cambia ninguna decisión, mantener el producto sólo como herramienta productiva."],
  ...
  ["M06","Alto","La captura puede funcionar offline y sincronizar sin perder orden ni evidencia.","Campo/feedlot no puede depender de red estable.","Piloto con cortes, reloj desajustado, duplicados y conflictos.","Si falla, usar appliance local o servicio operado con subida posterior controlada."],
  ["M07","Medio","La mayoría de caravanas SNIG objetivo es legible en el punto de captura.","Todas las rutas individuales dependen de identidad electrónica útil.","Medir tasa de lectura por categoría, dispositivo y geometría.","Diseñar excepción visual/manual y costo explícito de reidentificación."],
  ...
  ["M09","Medio","Los proveedores globales no ofrecen ya el paquete completo y soportado en Uruguay.","Optiweigh declara presencia local y Datamars tiene representación de marca.","Diligencia comercial con Olho do Dono, Optiweigh, Datamars y Vytelle.","Si existe oferta completa, diferenciar por servicio/auditoría o asociarse."],
  ...
  ["M11","Medio","Un mismo modelo de evento sirve a feedlot y auditoría aunque cambie el hardware.","La secuencia del proyecto depende de reutilizar identidad y evidencia.","Mapear campos obligatorios y decisiones de ambos contextos.","Separar productos y contratos si los estándares probatorios divergen demasiado."],
...
function assumptionsPage() {
  ...
  return `${head("Supuestos de mercado · Ganadería aumentada", "Supuestos que sostienen el mapa competitivo y su posicionamiento.")}${nav("mercado")}<main><section class="hero hero--compact"><div><p class="breadcrumbs"><a href="mercado.html">Mapa competitivo</a> / Supuestos</p><p class="eyebrow">Documento M‑A · Supuestos</p><h1>El posicionamiento también es una hipótesis.</h1><p class="lede">Doce afirmaciones que conectan el mapa competitivo con decisiones de producto. Cada una declara cómo probarla y qué cambiar si resulta falsa.</p></div><aside class="meta-card"><dl><dt>Supuestos</dt><dd>12</dd><dt>Críticos</dt><dd>3</dd><dt>Fecha</dt><dd>23 ago 2026</dd><dt>Documento base</dt><dd><a href="supuestos.html">S01–S26</a></dd></dl></aside></section><div class="layout"><aside class="toc"><strong>Prioridad</strong><a href="#m01">M01 · Diferencial</a><a href="#m02">M02 · Paso</a><a href="#m03">M03 · Correspondencia</a><a href="#m04">M04 · Valor probatorio</a><a href="#m09">M09 · Oferta local</a></aside><article class="content"><section class="section"><div class="callout"><strong>Uso:</strong> no aprobar un roadmap porque “parece razonable”. Convertir cada supuesto crítico en entrevista, dato o prueba y registrar la respuesta en esta misma colección.</div>${rows}</section></article></div></main>${footer()}</body></html>`;
}
...
const marketQuestions = [
  ...
  ["MQ18","Proveedor","¿Qué compatibilidad certifica con HDX/ISO y con caravanas SNIG reales?","Proveedor de hardware","Evita inferir compatibilidad por frecuencia."],
...
function questionsPage() {
  ...
  return `${head("Preguntas de mercado · Ganadería aumentada", "Preguntas para mejorar el análisis competitivo y diseñar diligencia y pilotos.")}${nav("mercado")}<main><section class="hero hero--compact"><div><p class="breadcrumbs"><a href="mercado.html">Mapa competitivo</a> / Preguntas</p><p class="eyebrow">Documento M‑Q · Preguntas</p><h1>Preguntar lo que cambia la decisión.</h1><p class="lede">Veinticuatro preguntas para clientes, proveedores, financiadores y pilotos. Priorizan evidencia, interoperabilidad y costo total por encima de la demo.</p></div><aside class="meta-card"><dl><dt>Preguntas</dt><dd>24</dd><dt>P0</dt><dd>8</dd><dt>Proveedor</dt><dd>4</dd><dt>Fecha</dt><dd>23 ago 2026</dd></dl></aside></section><div class="layout"><aside class="toc"><strong>Bloques</strong><a href="#mq01">Comprador y mercado</a><a href="#mq09">Procedencia</a><a href="#mq17">Proveedor</a><a href="#mq21">Piloto</a><a href="preguntas.html">Preguntas originales</a></aside><article class="content"><section class="section"><div class="callout"><strong>Regla de diligencia:</strong> pedir archivos, muestras, contratos o una prueba observable. Una respuesta comercial sin evidencia queda marcada como hipótesis.</div>${rows}</section></article></div></main>${footer()}</body></html>`;
}
